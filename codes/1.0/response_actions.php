				...
				if ($conn)
				{
						...
						if (i5_num_rows($result) > 0)
						{
							...
							foreach ($row['orderItems'] as $item)
							{
								...
								if ($orderItemId <> null)
								{
									...
									$newRecord = array($orderId, $orderItemId, $sku, $productName, $price, $basePrice, $weight, $qty, $qty, $subtotal, $baseSubtotal,  
					...
					if ($orderId > 0)
					{
						...
						if ($ret == false)
						{
							...
							$errmsg = $err[3];
							//if failed to call program write a log
							die(MessageHandler("EC104R program call failed from response_actions.php, function ProcessNewOrder:  order#: $orderId, Message: $errmsg, Library List: $liblist"));			   
						}		

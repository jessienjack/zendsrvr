			...
			if ($conn)
			{
				...
				if ($customerId == null)  
				{
					...
					if (i5_num_rows($result) > 0)
					{
						...
						$sql = 'INSERT INTO RECORHP (OHORID, OHTIME, OHTYPE, OHPID, OHSTS, OHCUID, OHCNAM, OHCEMA, OHCGRP, OHCUR, 
													 OHSTOT, OHBTOT, OHDIS, OHBDIS, OHSHP, ,OHBSHP, OHGTOT, OHBGTO, OHPAID, OHBPAI,
													 OHITEM, OHQTY, OHWHT,  OHPAYM, OHPYID, OHPYTM, OHRECP, OHSHPM, OHSHPA, OHSHPP,
													 OHTEL,  OHMOB,  OHREM1, OHPIC)'
							. ' VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
						$req = i5_prepare($sql);
						
						$newRecord = array($orderId, $createTime, $type, $parentId, $status, $customerId, $customerName, $customerEmail, $customerGroup, $currency, 
										   $subTotal, $baseSubTotal, $discountAmount, $baseDiscountAmount, $shippingAmount, $baseShippingAmount, $grandTotal, $baseGrandTotal, $totalPaid, $baseTotalPaid, 
										   $itemCount, $totalQty, $weight, $paymentMethod, $paymentTransactionId, $payTime, $recipient, $shippingMethod, $shippingAddress, $postcode, 

						...
						if (i5_num_rows($result) > 0)
						{
							...
							foreach ($row['orderItems'] as $item)
							{
								...
								if ($orderItemId <> null)
								{
									...
									$zero   = 0;
									// Write Detail Record ...
									$sql = 'INSERT INTO RECORDP (ODORID, ODITID, ODSKU, ODPNAM, ODPRC, ODBPRC, ODWHT, ODQTY, ODSTOT, ODBSTO, ODDIS, ODBDIS, ODRWHT, ODTOT, ODBTOT, ODOSTR, ODESTR, ODDDAT, ODDTIM, ODCARR, ODTCOD)' 
									. ' VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
									$req = i5_prepare($sql);

	...
	foreach ( $arraydata as $rowdata) 
	{
		if ($rowdata[0] == 'success')
		{
			...
			foreach ($array as $rowsuccess)
			{
				...
				// Set ESREDY = "N" for successful SKU, so that it doesn't get built next time
						...
						if (i5_num_rows($result) > 0)
						{
							...
							foreach ($row['orderItems'] as $item)
							{
								...
								if ($orderItemId <> null)
								{
									...
									$nodate = 0;
									// Write Detail Record ...
									$sql = 'INSERT INTO RECORDP (ODORID, ODITID, ODSKU, ODPNAM, ODPRC, ODBPRC, ODWHT, ODQTY, ODSTOT, ODBSTO, ODDIS, ODBDIS, ODRWHT, ODTOT, ODBTOT, ODOSTR, ODESTR, ODDDAT)' 
									. ' VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
									$req = i5_prepare($sql);
									
									$newRecord = array($orderId, $orderItemId, $sku, $productName, $price, $basePrice, $weight, $qty, $subtotal, $baseSubtotal,  
													   $discountAmount, $baseDiscountAmount, $rowWeight, $rowTotal, $baseRowTotal, $store, $store, $nodate);
									$result = i5_execute($req, $newRecord);
